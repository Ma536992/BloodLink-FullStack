...
class BloodRepository {
    ...
    private val requestsRef = db.child("emergency_requests") // Synced with Web Admin
    private val messagesRef = db.child("messages")
    private val inventoryRef = db.child("inventory/global")
    ...
    suspend fun acceptRequestAsDonor(requestId: String, donorId: String, donorName: String) {
        val updates = mapOf(
            "status" to "Accepted",
            "acceptedByUserId" to donorId,
            "acceptedByUserName" to donorName,
            "acceptedBy" to donorName // Also update for web compatibility
        )
        requestsRef.child(requestId).updateChildren(updates).await()
    }

    fun getLiveEmergencyFeed(currentUserId: String): Flow<List<BloodRequest>> = callbackFlow {
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val requests = snapshot.children.mapNotNull { 
                    try {
                        it.getValue(BloodRequest::class.java)
                    } catch (e: Exception) {
                        null
                    }
                }
                    .filter { it.status == "Pending" && it.requesterId != currentUserId }
    ...
    fun getMyRequests(userId: String): Flow<List<BloodRequest>> = callbackFlow {
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val requests = snapshot.children.mapNotNull { 
                    try {
                        val req = it.getValue(BloodRequest::class.java)
                        req?.copy(id = it.key ?: "")
                    } catch (e: Exception) {
                        null
                    }
    ...
    fun getGlobalInventory(): Flow<Map<String, Int>> = callbackFlow {
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
